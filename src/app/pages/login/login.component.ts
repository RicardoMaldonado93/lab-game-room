...
export class LoginComponent implements AfterViewInit, OnInit {
  ...
  async onSubmit() {
    ...
    const { status, message } = await this.auth.loginWithEmailAndPassword({
      user,
      pass,
    });
    if (status) {
  ...
  async onSubmitRegister() {
    ...
    const { status, message } =
      await this.auth.createNewUserWithEmailAndPassword(data);
    if (status) {
  ...
  private formInicialization() {
    ...
    this.loginForm = new FormGroup({
      user: new FormControl('', [Validators.required, this.__emailValidator]),
